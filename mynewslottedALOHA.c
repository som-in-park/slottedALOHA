...
int main() {
	...
	*/
	for (int nUE = 0; nUE < 4; nUE++) {
		...
		for (int t = 0; t < 60000; t++) {
			...
			*/
			...
			for (int u = 0; u < numofUE[nUE]; u++) {
				if (UE_state[u][1] != 0)
					UE_state[u][1]--;
			}

			for (int u = 0; u < nextUE; u++)	UE_state[u][3]++;
			...
			//printf("**");
			//for (int ue = 0; ue < UEpertime; ue++) {
			//	printf("%2d  ", UE_Tx[ue][0]);
			//}
			//printf("**\n");


			int cnt = 0;
			//for (int ue = 0; ue < numofUE[nUE]; ue++) {
			//	cnt++;
			//	printf("|%2d: %d| ", ue, UE_state[ue][1]);
			//	if (cnt % 10 == 0)printf("\n");
			//}
			//printf("\n");
		...
		double avg_tx = (double)total_sc_count / (double)total_success;
		double suc_rate = (double)total_success / (double)numofUE[nUE];
		double col_rate = (double)total_col / (double)total_tx;
		double avg_time = (double)total_sc_time / (double)total_success;
	...
	for (int nUE = 0; nUE < 4; nUE++) {
		...
		printf("[ UE: %d, slot: %d, UEpertime: %d ]\n", numofUE[nUE], numofslot, UEpertime);
		//printf("%d, (success: %.0f, fail: %.0f, left UE: %.0f, avg delay: %f), %f\n", time, result[nUE][0], result[nUE][1], result[nUE][2], result[nUE][3], (double)result[nUE][0] / numofUE[nUE]);
		printf("(success rate: %f, collision rate: %f, avg tx: %f, avg delay: %f)\n", result[nUE][3], result[nUE][4], result[nUE][5], result[nUE][6]);
		printf("\n");
	}
}